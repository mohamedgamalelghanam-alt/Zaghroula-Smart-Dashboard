...
# ---------------------------------------------------
# قراءة وتنظيف البيانات (التعديل الجوهري هنا)
# ---------------------------------------------------
@st.cache_data
def load_data(file):
    # محاولة القراءة بكل الطرق الممكنة (عشان الملف الخداع)
    try:
...
if uploaded_file:
    ...
    with tab4:
        ...
        **1. Dashboard 📊**
        - يعرض ملخص اليوم:
          - إجمالي المبيعات
          - صافي الأرباح (محسوب بخصم س شراء من إجمالي البيع)
          - عدد الأصناف الناقصة
        ...
        **3. Reports 📝**
        - تحميل تقرير Word جاهز.

        ### ملاحظات مهمة:
        - الحسبة الآن: **إجمالي البيع - إجمالي التكلفة (س شراء)**.
        - يتم استبعاد صفوف "وارد" و "اجمالى" تلقائياً لضمان دقة الأرقام.
        """)
