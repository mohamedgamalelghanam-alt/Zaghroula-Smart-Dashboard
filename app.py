...
if uploaded_file:
    # 2. وظيفة التنظيف والحسابات (اللي جربناها في كولاب)
    @st.cache_data # عشان البرنامج يبقى سريع
    ...
    # 3. عرض المؤشرات الرئيسية (Key Metrics)
    col1, col2, col3 = st.columns(3)
    col1.metric("💰 إجمالي المبيعات", f"{data['Total_Sales'].sum():,.2f} ج.م")
    col2.metric("📈 صافي الأرباح", f"{data['Net_Profit'].sum():,.2f} ج.م")
